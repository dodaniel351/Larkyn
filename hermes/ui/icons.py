...
The icon is tinted by pipeline state so the tray reflects what Hermes is doing.
"""
...
from hermes.core.interfaces import PipelineState
...
def app_icon() -> QIcon:
    """The branded application icon (assets/hermes.ico), with fallback."""
    path = asset_path("hermes.ico")
    if path:
        return QIcon(path)
