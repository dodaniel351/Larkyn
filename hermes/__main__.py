...
from hermes.audio.capture import SoundDeviceCapture
from hermes.config import AppConfig, history_db_path
...
from hermes.output.sink import SystemOutputSink
...
def start_hotkey(bridge: HotkeyBridge, hotkey: str):
    from hermes.hotkey import HotkeyListener
...
def main() -> int:
    ...
    # --- UI -------------------------------------------------------------------
    from hermes.ui.context import SignalBus, UiContext
    from hermes.ui.mainwindow import MainWindow
    ...
    app._hermes_refs = (orchestrator, overlay, tray, bridge, window, ctx, shm, state)  # type: ignore[attr-defined]
